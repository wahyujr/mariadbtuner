...
##          $myvar{'max_allowed_packet'} +  ##not used for calculation
	...
	#infoprint " ";
	#infoprint "---------------- Thread Buffer per-Connection: ------------------" ;
	#infoprint "read_buffer_size       : " . $myvar{'read_buffer_size'}. " \t(".$myvar{'read_buffer_size'}/1024 ."\tK)" ;
	#infoprint "read_rnd_buffer_size   : " . $myvar{'read_rnd_buffer_size'}." \t(".$myvar{'read_rnd_buffer_size'}/1024 ."\tK)";
	#infoprint "sort_buffer_size       : " . $myvar{'sort_buffer_size'}." \t(". $myvar{'sort_buffer_size'}/1024/1024 ."\tM)";
	#infoprint "thread_stack           : " . $myvar{'thread_stack'}." \t(". $myvar{'thread_stack'}/1024 . "\tK)";
	#infoprint "max_allowed_packet     : " . $myvar{'max_allowed_packet'}. " \t(". $myvar{'max_allowed_packet'}/1024/1024 ."\tM)";
	#infoprint "join_buffer_size       : " . $myvar{'join_buffer_size'}." \t(". $myvar{'join_buffer_size'}/1024 . "\tK)";
	#infoprint "---------------- Thread Buffer ----------------------------------" ;
    }
    else {
        $mycalc{'per_thread_buffers'} =
          $myvar{'record_buffer'} +
...
infoprint " ".hr_bytes($myvar{'innodb_buffer_pool_size'})." + ".hr_bytes($myvar{'key_buffer_size'})." + (".$myvar{'max_connections'}." * ". hr_bytes($mycalc{'per_thread_buffers'}). "->". hr_bytes($mycalc{'per_thread_buffers'}*$myvar{'max_connections'}) .") = ".hr_bytes($myvar{'innodb_buffer_pool_size'}+$myvar{'key_buffer_size'}+($mycalc{'per_thread_buffers'}*$myvar{'max_connections'}))." ";
infoprint "------- WJR Total RAM Needed --------- " ;
